...
/**
 * LoginRadiusSocialLoginAndSocialShare Model.
 */
class LoginRadiusSocialLoginAndSocialShareModelLoginRadiusSocialLoginAndSocialShare extends JModelList
{
    ...
    public function saveSettings()
    {
        ...
        $settings = $application->input->getVar('settings');
        ...
        $settings['horizontalarticles'] = (sizeof($application->input->getVar('horizontalarticles')) > 0 ? serialize($application->input->getVar('horizontalarticles')) : "");
        $settings['verticalarticles'] = (sizeof($application->input->getVar('verticalarticles')) > 0 ? serialize($application->input->getVar('verticalarticles')) : "");
        $settings['horizontalrearrange'] = (sizeof($application->input->getVar('horizontalrearrange')) > 0 ? serialize($application->input->getVar('horizontalrearrange')) : $defaultShare);
        $settings['verticalrearrange'] = (sizeof($application->input->getVar('verticalrearrange')) > 0 ? serialize($application->input->getVar('verticalrearrange')) : $defaultShare);
        $settings['horizontalcounter'] = (sizeof($application->input->getVar('horizontalcounter')) > 0 ? serialize($application->input->getVar('horizontalcounter')) : $defaultCounter);
        $settings['verticalcounter'] = (sizeof($application->input->getVar('verticalcounter')) > 0 ? serialize($application->input->getVar('verticalcounter')) : $defaultCounter);
    ...
    private function horizontalShare($settings)
    {
        ...
        {
            $sharescript = 'LoginRadius.util.ready(function () {$i = $SS.Interface.' . $interface . '; $SS.Providers.Top = ' . json_encode(@unserialize($settings['horizontalrearrange'])) . '; $u = LoginRadius.user_settings; $u.sharecounttype = \'url\'; $u.apikey = "' . $settings['apikey'] . '"; $u.isMobileFriendly = true; $i.size = ' . $size . ';$i.show("lrsharecontainer"); });';
    ...
    private function verticalShare($settings)
    {
        ...
        if (isset($size) && !empty($size))
        {
            $vsharescript = 'LoginRadius.util.ready(function () {$i = $SS.Interface.' . $vinterface . '; $SS.Providers.Top = ' . json_encode(@unserialize($settings['verticalrearrange'])) . '; $u = LoginRadius.user_settings; $u.apikey = "' . $settings['apikey'] . '"; $u.isMobileFriendly = true; $i.size = ' . $size . ';$i.left = "' . $vershreleft . '"; $i.top = "' . $vershretop . '";$i.right = "' . $vershreright . '";$i.bottom = "' . $vershrebottom . '"; $i.show("lrverticalsharecontainer"); });';
