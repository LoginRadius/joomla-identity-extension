...
class plgSystemSocialLoginFunctions
{
    ...
    public static function makeKunenaUser($userId, $profileImage, $userImage, $profileData)
    {
        if (JPluginHelper::isEnabled('system', 'kunena'))
        {
            ...
            {
                $kunenaQuery = "INSERT IGNORE INTO #__kunena_users (`userid`,`avatar`,`gender`,`birthdate`,`location`,`personalText`,`websiteurl`) VALUES(" . $db->Quote($userId) . "," . $db->Quote($dumpUserImage) . "," . $db->Quote($profileData['gender']) . "," . $db->Quote($profileData['dob']) . "," . $db->Quote($profileData['city']) . "," . $db->Quote($profileData['aboutme']) . "," . $db->Quote($profileData['website']) . ")";
            }
    ...
    public static function makeJomSocialUser($userId, $profileImage, $userImage)
    {
        if (JPluginHelper::isEnabled('system', 'jomsocialconnect'))
        {
            $db = JFactory::getDBO();
                ...
                {
                    $jsQuery = "INSERT IGNORE INTO #__community_users(`userid`,`avatar`,`thumb`) VALUES(" . $db->Quote($userId) . "," . $db->Quote($avatar) . "," . $db->Quote($avatar) . ")";
                }
                $db->setQuery($jsQuery);
                $db->query();
        }
