...
class plgSystemSocialLoginTools
{
    ...
    public static function emailPopup($msg, $msgTitle, $msgType)
    {
        ...
        $document->addStyleSheet(JURI::root() . 'plugins/system/socialloginandsocialshare/assets/popupstyle.css');
    ...
    public static function addNewIdImage($thumbnail, $socialId)
    {
        if (empty($thumbnail))
        {
            $thumbnail = JURI::root() . 'media' . DS . 'com_socialloginandsocialshare' . DS . 'images' . DS . 'noimage.png';
        }
    ...
    public static function insertUserPicture($path, $profileImage, $userImage)
    {
        ...
        } else
        {
            ...
            if (empty($thumbImage))
            {
                $thumbImage = @file_get_contents(JURI::root() . 'media' . DS . 'com_socialloginandsocialshare' . DS . 'images' . DS . 'noimage.png');
            }
    ...
    public static function sendMail(&$user, $userMessage)
    {
        ...
        if ($return !== true)
        {
            JError::raiseWarning('', JText::_('COM_USERS_REGISTRATION_SEND_MAIL_FAILED'));
