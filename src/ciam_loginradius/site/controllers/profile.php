 ...
 * @since        1.6
 */
class CiamLoginRadiusControllerProfile extends CiamLoginRadiusController
{
    ...
    public function save()
    {
        ...
        $this->setRedirect(JRoute::_('index.php?option=com_ciamloginradius&view=updateprofile', false));       
        return;
    }
    ...
    public function removeSocialAccount()
    {
        ...
        $app = JFactory::getApplication();
        $model = $this->getModel('Profile', 'CiamLoginRadiusModel');
        $user = JFactory::getUser();
        $userId = (int) $user->get('id');
        if ($user->get('guest') == 1)
        {
        ...
        if ($deleted == true)
        {
            $this->setMessage(JText::_('COM_CIAM_LINK_ACCOUNT_DELETE'));
