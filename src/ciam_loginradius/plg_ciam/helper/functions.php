...
class plgSystemCiamFunctions
{
    ...
    public static function getExistUserName($username)
    {
        ...
        return plgSystemCiamFunctions::removeUnescapedChar($userName);
    }
    /**
     * @param $profileData
     * @return mixed
    ...
    {
        $profileImage = !empty($userProfile->thumbnail) ? $userProfile->thumbnail : JURI::root() . 'media' . DS . 'com_ciamloginradius' . DS . 'images' . DS . 'noimage.png';
    ...
    public static function makeComProfileUser($userId, $profileImage, $userImage, $profileData)
    {
        ...
        if (JPluginHelper::isEnabled('system', 'communitybuilder'))
        {
            ...
            plgSystemCiamTools::insertUserPicture($path, $profileImage, $userImage);
            plgSystemCiamTools::insertUserPicture($path, $profileImage, 'tn'.$userImage);
            $query = $db->getQuery(true)
                            ->select('id')
