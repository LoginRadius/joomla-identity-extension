...
class CustomHttpClient  implements IHttpClient {
    ...
    if ($query_array !== false) {
      ...
      $requestedData = array( 'GET' => $query_array,
        ...
        $db = JFactory::getDbo(); 
        $columns = array('api_url', 'request_type', 'data', 'response', 'response_type', 'timestamp');
                $values = array($db->quote($request_url), $db->quote($method), $db->quote(json_encode($requestedData)) , $db->quote(json_encode($response)), $db->quote(ucfirst($response_type)), $db->quote(time()));
                $query = $db->getQuery(true)
                        ->insert($db->quoteName('#__loginradius_log'))
