...
require_once (JPATH_ROOT.DS. 'plugins'. DS . 'system'. DS .'userregistration'. DS .'userregistration.php');

/**
 * Controller Class UserRegistrationAndManagementController
 */
class UserRegistrationAndManagementController extends LRController
{
    ...
    public function view()
    {
        $mainframe = JFactory::getApplication();
        $option = JRequest::getVar('option', 'userregistrationandmanagement');
        $userid = JRequest::getVar('cid', array(), 'userregistrationandmanagement');
        if (sizeof($userid) == 1):
            $this->setRedirect(JRoute::_('index.php?option=' . $option . '&view=userprofile&userid=' . $userid[0], false));
        else:
            $mainframe->enqueueMessage(JText::_('COM_SOCIALLOGIN_USER_PROFILE_ERROR'));
            $this->setRedirect(JRoute::_('index.php?option=' . $option . '&view=usermanager', false));
        endif;
    }
    /**
     * edit user profile from user manager 
     */
    ...
    {      
        ...
        $mainframe->redirect('index.php?option=com_users&view=user&task=user.edit&id=' . $cid[0]);
    }

    /**
     * Remove user account from user manager 
     */
    function remove()
    {
        JRequest::checkToken() or jexit('Invalid Token');
    ...
    /**
     * block user account from user manager 
     */
    function block()
    {
        JRequest::checkToken() or jexit('Invalid Token');
    ...
    /**
     * publish user account from user manager 
     */
    function publish()
    {
        JRequest::checkToken() or jexit('Invalid Token');
    ...
    /**
     * create user account from user manager 
     */
    public function add()
    {   
