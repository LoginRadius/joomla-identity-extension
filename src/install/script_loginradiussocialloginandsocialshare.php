...
/**
 * Script file of loginradiussocialloginandsocialshare component
 */
class Com_LoginRadiusSocialLoginAndSocialShareInstallerScript
{
     ...
     */
    public function installationScript($parent, $action)
    {
        $status = new stdClass;
        ...
        $isUpdate = JFile::exists(JPATH_SITE . '/modules/mod_sociallogin/mod_sociallogin.php');
        // create a folder inside your images folder
        JFolder::create(JPATH_ROOT . '/images/sociallogin');

        // Load sociallogin language file
        $lang = JFactory::getLanguage();
        $lang->load('com_loginradiussocialloginandsocialshare', JPATH_SITE);
        ...
        foreach ($modules AS $module)
        {
            ...
            $path = $src . '/' . $moduleData ['module'];
            $installer = new JInstaller;
            $result = $installer->$action($path);

            if ($result)
            {
                $status->modules[] = array('name' => $moduleData ['title'], 'client' => $moduleData ['client']->name, 'result' => $result);
            }
        }
        ...
        foreach ($plugins AS $plugin)
        {
            ...
            $path = $src . '/' . 'plg_' . $pluginData['plugin'];
            $installer = new JInstaller;
            $result = $installer->$action($path);
            ...
            // Plugin Installed
            $status->plugins[] = array('name' => $pluginData ['title'], 'group' => $pluginData ['group']);
            if ($action == 'update')
            {
     ...
     * @return bool
     */
    public static function updatePlugin($parent, $pluginData)
    {
        $db = JFactory::getDBO();
     ...
     * @param $status
     */
    private static function installationResults($status)
    {
        $rows = 0;
        if (count($status->modules) AND count($status->plugins))
        {
            ?>
            <h2>LoginRadius Social Login and Social Share Installation</h2>
            <table class="adminlist table table-striped">
                ...
                <tbody>
                    ...
                    <tr class="row0">
                        <td class="key"
                            colspan="2"><?php echo 'LoginRadius Social Login and Social Share ' . JText::_('Component'); ?></td>
            ...
            <h2>LoginRadius <?php echo JText::_('COM_SOCIALLOGIN_INSTALLATION_STATUS'); ?></h2>
            <p class="nowarning">
            <?php echo JText::_('COM_SOCIALLOGIN_INSTALLATION_THANK'); ?> <strong>LoginRadius Social Login and Social Share</strong>!
            <?php echo JText::_('COM_SOCIALLOGIN_INSTALLATION_CONFIG'); ?>
                <a href="index.php?option=com_loginradiussocialloginandsocialshare">LoginRadius Social Login and Social Share</a> <?php echo JText::_('COM_SOCIALLOGIN_INSTALLATION_COM'); ?>
